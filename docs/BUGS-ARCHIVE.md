...
### BR66.01 — Resolved — owner: `CC`
**`MapCorpus.forget()` clears `fills`, so a shard can accrue maps it never ledgers**
- **cluster:** `suite`
- **2026-08-12 — RESOLVED (taskblock-66 addendum follow-up).** `forget()` now clears `_cache` and
  `generated` and **leaves `fills`**; a new `MapCorpus.forget_ledger()` clears all three and is used
  only by `test_shard_merge.gd:14,95,101`, whose subject genuinely is two separate processes. The
  ripple recorded below was the whole difficulty and it held: nothing else asserts on `fills`, and
  the four `generated` assertions are untouched because `forget()` still resets that counter.
- **The reproduction no longer reproduces.** `test_generated_board_sight_sweep.gd` plus
  `test_work_counters.gd` in one process ledgered `{}` against 12 maps before; it now ledgers
  `{"3:12:10": …, "642296523:40:30": …}`.
- **And the gate now prints a duplication line where it printed none.** Post-fix sharded gate: 366
  scripts, **3565 tests**, 0 failures — *"duplicated corpus fills subtracted: maps 1, floods 2 (155
  distinct keys)"*. Pre-fix it printed nothing, which `merge_shards.py` does only when duplication is
  zero.
- **The honest size of the defect, which is smaller than the entry feared.** Real duplication on the
  current shard map is **1 map and 2 floods**, so `maps` reads 883 controlled against 884 raw. **Both
  halves of the original ambiguity were true at once**: the ledger really was being wiped, *and* the
  packer's co-location really is good. The reported zero was a wiped ledger sitting on top of a
  near-zero true value. **The fix matters as insurance against a future repack rather than as a
  correction to today's totals**, and saying otherwise would oversell it.
- **Tests:** `test_map_corpus.gd::test_forget_keeps_the_ledger_of_what_this_process_already_generated`
  asserts against **`MapGen.maps_generated`**, deliberately, since the gated counter is the one that
  must not disagree with the ledger; `MapCorpus.generated` is reset by `forget()` by design and
  asserting on it would hide the bug. Paired with
  `test_forget_ledger_wipes_the_record_as_well` so the other half cannot silently regress.
- **2026-08-12 — CONFIRMED `Active`, reproduced in isolation (taskblock-66 addendum item A).**
  The mechanism was read out of the code when this was filed; it is now observed. Two runs of the
  same file, `test_generated_board_sight_sweep.gd`, which reads `MapCorpus` and does **not** forget:

  | run | `maps` | `fills` |
  |---|---:|---|
  | the file alone | 1 | `{"642296523:40:30": {floods 2, maps 1}}` |
  | the file **plus `test_work_counters.gd`** in one process | 12 | `{}` |

  **The first run proves the fill is ledgerable; the second proves the ledger is wiped while the
  gated counter keeps the map.** That is exactly under-subtraction: `ShardMerge.duplication` unions
  per-shard `fills`, and a wiped ledger contributes nothing to the union.
- **The sharded gate reads consistent with it.** 366 scripts, 3563 tests, 0 failures, N=1 draw.
  `merge_shards.py` prints its duplication line only when duplication is non-zero and **printed
  nothing**, i.e. zero — while shard 2 carries **twelve** `MapCorpus`-referencing files and shard 7
  two. Per-shard ledger sizes: shard 2 has 103 `fills` keys, shard 4 has 50, shard 1 has 1, and
  **shards 0, 3, 5, 6 and 7 have none**. Shard 7 holds `test_work_counters.gd` (which forgets) beside
  `test_generated_board_sight_sweep.gd` (which fills) and ledgers **zero**, which is the isolated
  reproduction happening inside the real gate.
- **Do not read the raw `maps` minus ledger difference as the under-count.** Most `maps` are
  generated by tests that never touch `MapCorpus`, so that difference (724 of 878 across the gate) is
  mostly legitimate non-corpus generation. **The defect is the shape above — a fill that happened and
  is absent from the ledger — not the size of that gap.**
- **Not fixed here, deliberately.** The addendum's A2 says fix only if trivial in the same pass, and
  it is not: `forget()` also zeroes `generated`, and **four assertions depend on that** —
  `test_map_corpus.gd:28,37,84` and `test_shard_merge.gd:97,103` read `MapCorpus.generated` around
  `forget()` calls. So the split cannot be a one-line edit to `fills.clear()`; `forget()` must keep
  clearing `_cache` and `generated` while leaving `fills`, and `test_shard_merge.gd:14,93,99` must
  move to a new `forget_ledger()` that clears all three, or its process-boundary simulation
  accumulates across its own tests. **Recorded so the fixing pass does not re-derive it.**
