...
### BR65.01 — Active — owner: `CC`
**`FIRST_WIN_CAP` was sized for a ~72% completion rate and the real rate is ~15%, so the full
gate's completion test fails about one run in four**
- **cluster:** `test-suite`
...
was a game rules update, and ~40% down to 15% was a map update."* **This entry is only about the
threshold that was never resized to follow**, and the first version of it had that backwards.

**The measurement**, from `tools/probe_seeds.gd` over seeds 0–19:

```
completion 3/20 (15.0%), mean 58.3 turns to complete
{ "STRANDED": 11, "EXTRACTED": 3, "TERMINATED": 6 }
```

Run again on a worktree of `60c9554` (tb64 Pass F): **byte-for-byte identical, same outcome and turn
count on all twenty seeds.** So nothing in tb65 moved it, which is what that comparison was for.

**The live defect.** `test_full_mission.gd::test_seeds_to_first_completion_stays_low` asserts
`sampled["won"]` after `CompletionSampler.seeds_to_first_win` tries up to `FIRST_WIN_CAP` (9)
clock-drawn seeds. **At a 15% rate, P(no win in 9) = 0.232 — roughly one full gate in four goes red
on this alone**, with nothing wrong. At the 0.72 the cap was chosen against it would have been 1 in
94 000. **Observed:** it fired once during tb65's close-out, and three clock-seeded draws in one
session went 7, 9-with-no-win, and 1 — unremarkable at 15%, near-impossible at 72%.

**Two recorded numbers now describe a game that no longer exists**, and both are load-bearing in
comments rather than in code:
- `MIN_COMPLETION_RATE = 0.35`, which `test_full_mission.gd` states is *"left in place and unused by
  this test"*. A floor at more than twice the real rate that nothing evaluates.
- `BoutCorpus`'s header argues a spurious dip is rare *"at the measured 0.72 completion rate"* —
  the premise the whole sample-here-measure-with-a-command design rests on.

**The fix is a resize, not an investigation.** To keep the false-red rate near 1 in 100 at a 15%
completion rate, the cap wants to be about **28** (`0.85^28 = 0.0106`); at 1 in 20 it is 18. That is
a decision about how much a green gate is worth paying for, since a raised cap costs bouts on
exactly the unlucky runs — which is why it is filed rather than picked. **Whatever number is chosen,
`FIRST_WIN_CAP` should carry the completion rate it was sized against**, so the next rules or map
change makes it visibly stale instead of quietly flaky.

**Caveat on my own number:** 20 seeds puts 15% at roughly 3–38% with 95% confidence, and
`ESCALATION_SEEDS` is 100. Run the full hundred before treating 15.0% as the figure to size against.
