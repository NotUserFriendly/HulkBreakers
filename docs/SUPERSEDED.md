...
| A queued `MoveAction`'s own queue-row text was its full `describe()` — the whole path, unbounded in length | `CombatAction.short_describe()` (new; defaults to `describe()`) — `MoveAction` overrides it to just "Move"; the full path only reaches the hover tooltip, as an extra "Detail" row | BR27.08 (supervisor follow-up) |
