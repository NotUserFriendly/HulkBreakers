...
untracking, or a pointer moved onto the file that actually holds the value.

### Pass A — taskblock specs stop reaching git

**20 spec files untracked, all still on disk.** `taskblock_done/` (16), the three root specs, and
`docs/taskblock61.md`. `.gitignore` line 23 had ignored `/taskblock_done/` since it was written while
every file in it stayed tracked — an ignore rule does not untrack what is already committed — and the
root specs had no rule at all. **The rule was inverted from practice in both directions.** It now
carries `/taskblock*.md` and `/taskblock_done/` with the reason: a spec whose passes were never
executed reads as a record of work that did not happen. `reports/` is the record.

**`docs/taskblock61.md` was not in the block's enumeration** — it was found by the Pass A2 sweep and
moved to `taskblock_done/taskblock61.md` on the supervisor's instruction (*"61 doesn't belong in
docs, it can be moved to done"*), which also closed the one gap in that directory's sequence. Its
triage content was already duplicated at `docs/BUGS.md:149`.

**Three dangling path references carried inline instead of pointed at:**
- `wall_cutout.gdshader` cited `taskblock_done/Report-Taskblock32.md` for the same-side over-cutting
  writeup. That report is five blocks past the rolling window. The fact is `BR32.05`, `Resolved` on
  the owner's word at tb61 Pass C and archived with the elevation analysis and two candidate fixes —
  **with this specific per-fragment residual closed unaddressed rather than fixed**. The comment
  said "still open" for a question that had been closed.
- `test_tb38_flat_bout_guard.gd` cited `docs/taskblock38.md`; the charter it quotes was already
  inline, so only the path went.
- PLAN's log-rotation item cited `taskblock_done/` as a second rolling-window precedent. There is
  now one: `reports/`.

### Pass B — stale pointers in the living docs

**B1: the audit index path.** PLAN said `test/suite_audit.csv`; it is `audit/suite_audit.csv`
(`tools/run_suite.gd:97`), which `CLAUDE.md` item 5b and `docs/TEST-AUDIT.md` already stated
correctly. Three places, one of them disagreeing.
...
**Discarded with it, recorded here rather than lost:** the observation that `board_view.gd` (1016) is
not bloated but is doing three jobs — static board geometry, the TACTICS overlay, and the wall-cutout
uniform feed — and that the cutout feed is the most separable of the three. That was never a
consequence of the cap; it simply has no forward work attached to it now.

**B3: live files state the cap's value, not its source.** Ten comments across `src/` and `test/`
labelled the gate by its old number. Where the number was only a label it is now
`gdlintrc`'s `max-file-lines`; where it was half of a tagged measurement (`inspect_panel.gd` at 992
of 1000, `injection_events.gd` at 998 → 1026, `board_view.gd` at exactly 1000) the measurement stays
and says which cap it was measured against. **`CHANGELOG` entries were deliberately not swept** —
they are tagged history and the bump itself is logged here; and `test_lint_config.gd` pins the live
2500 by design, so touching it would be changing a rule.

**Found by the B3 sweep, not in the block:** `SUITE-PROFILE.md`'s own header said *"Generated by
`tools/profile_suite.gd`"*. **That file was deleted at tb48 Pass A**, when three rungs collapsed onto
one runner; the generator has been `tools/run_suite.gd` for eighteen taskblocks. The artifact had
been misnaming its own source that whole time, in the one line a reader would use to regenerate it.
Fixed at the generator. **The same dead filename is quoted in `Report-Taskblock66.md`'s E6 note** —
left as written, since reports are appended to rather than revised, and flagged in this block's
report appendix instead.

**B4: no live doc still calls `test_watched_run.gd` an unexplained bottleneck.** Checked rather than
assumed: the two `CHANGELOG` mentions are tagged history that state the cause, `SuiteBudget`'s header
already explains the clock-seeded draw and the 2 bouts/88 turns → 7/246 swing, and `TESTING.md` and
`PLAN.md` mention the file for other reasons entirely. **What was missing was in the generated
profile**, whose wall-clock table put the file on top with no note — so the generator now says in the
artifact that the first row carries the corpus draw and its seconds are a band, not a figure.

### Pass C — README and LICENSE

**Both supplied by the supervisor and placed, not authored.** The README was checked against the repo
on the block's three points and **corrected to the repo in three places**: the test count `3547` was
stale against the committed profile and now points at `test/SUITE-PROFILE.md` instead of restating a
number; the layout table was missing `src/resource_editor/`; and `docs/` was described as `00`–`11`,
...
carrying the cutout tag should.** Correct, and it overrules a call CC made deliberately and
documented — *"non-wall blockers (cover) still count, deliberately: the safe direction is keeping a
cutout."* **That reasoning was wrong.** A cutout kept alive over geometry that can never be cut is
not a safe answer; it is a hole with no visible cause. `MapGen` places six cover types as blockers
(`scrap_pile`, `goo_barrel`, `crate`, `pillar`, `forklift`, `barrel_pallet`), every one drawn with
`lit_material` and none of them cuttable.

**There was no cutout tag to read.** `BoardView` decided which blockers got the cutout material
with `part.id == &"wall"` — content identity hardcoded in code, which CLAUDE.md forbids, and which
meant a second cuttable terrain type would have needed a code edit. Rather than mirror that
hardcode into the logic layer, the tag it should always have been now exists: **`&"cutout"`,
authored on `wall.tres`, spelled once as `WallLegibility.CUTOUT_TAG`**, and read by **both** the
material assignment and the sight gate. "Is it drawn cuttable" and "does it count as occluding" are
one authored fact now and cannot disagree.

**`RayCaster.blocker_obstructed_among` gained a `required_tag` filter**, defaulting to no filter.
The vocabulary stays with the caller — this class knows what a tag is and nothing about which ones
mean what, the same way `exclude_parts` states membership without an opinion about why.

**Three tests, because one would not have caught the ways this can go wrong:** cover between camera
and unit does not keep the cutout (with a sanity assertion that the pillar genuinely *is* in the
way, so the test is about what counts rather than what hits); a tagged wall still does, so a change
that silences the gate outright cannot pass both; and a part with an id nobody wrote code for,
carrying the tag, occludes like a wall — the "addable as data, no code edit" rule made checkable.

**Deliberately not changed:** `BoardView`'s build-step counter still splits walls from cover on
`blocker.id == &"wall"`, and `AsciiRender` still picks its wall glyph that way. Those ask what the
map is made of rather than what the cutout may cut, and silently changing a diagnostic's meaning is
worse than two different questions having two answers. Recorded so it reads as a decision.

Cost unchanged within noise — 686 usec a frame above the geometry floor against 616 before. The
filter removes candidates rather than adding work.
