...
static func flood(grid: Grid, origin: Vector2i) -> Dictionary:
...
static func one_way_cells(grid: Grid, origin: Vector2i) -> Array[Vector2i]:
	var outward: Dictionary = flood(grid, origin)
	var inward: Dictionary = _reverse_flood(grid, origin)
...
static func _reverse_flood(grid: Grid, origin: Vector2i) -> Dictionary:
	var pathfinder := Pathfinder.new(grid)
	var seen: Dictionary = {origin: true}
	var frontier: Array[Vector2i] = [origin]
	while not frontier.is_empty():
...
static func stranding_cells(grid: Grid) -> Array[Vector2i]:
	var found: Dictionary = {}
	for spawn: Vector2i in spawn_cells(grid):
		for cell: Vector2i in one_way_cells(grid, spawn):
			found[cell] = true
	var cells: Array[Vector2i] = []
