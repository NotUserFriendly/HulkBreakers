...
##
## Three guards enforce retired vocabulary — `void` (tb40), the `HULK_` tooling prefix (tb50) and
## `tile` (tb54). Each had grown its own copy of the same recursive scan, and two of them had
## grown *different* ideas of what a word boundary is. This is the one implementation.
##
...
## in the tree — taskblock-54 found this for `tile` and taskblock-55 re-ran the other two.
##
...
## file explains three of them at once. That is not a hole in the sweep — it is the guard
