...
func _make_core_only_unit(cell: Vector2i, squad: int, matrix_id: StringName) -> Unit:
...
func test_destroying_a_part_removes_stat_mods_and_drops_contents() -> void:
	...
	assert_false(chassis.slots.has(Enums.SlotType.TORSO))
...
func test_destroying_core_disables_chassis_and_ejects_matrix_core() -> void:
	var grid := Grid.new(7, 7)
	var victim := _make_core_only_unit(Vector2i(3, 3), 0, &"victim_matrix")
	var ally := _make_core_only_unit(Vector2i(5, 5), 0, &"ally_matrix")  # pure-diagonal offset -> unique nearest neighbor
	var enemy := _make_core_only_unit(Vector2i(0, 0), 1, &"enemy_matrix")
	...
	var ejected: MatrixCore = items[0]
	assert_eq(ejected.matrix, victim.matrix)
...
func test_eject_tie_break_uses_lowest_row_major_cell_index() -> void:
	...
	var victim := _make_core_only_unit(Vector2i(3, 3), 0, &"victim_matrix")
	var ally := _make_core_only_unit(Vector2i(5, 4), 0, &"ally_matrix")
...
func test_eject_with_no_living_ally_falls_back_to_nearest_own_cell() -> void:
	var grid := Grid.new(7, 7)
	var victim := _make_core_only_unit(Vector2i(3, 3), 0, &"lone_matrix")
...
func test_ejected_matrix_core_is_recoverable_by_adjacent_ally() -> void:
	var grid := Grid.new(7, 7)
	var victim := _make_core_only_unit(Vector2i(3, 3), 0, &"victim_matrix")
	var ally := _make_core_only_unit(Vector2i(5, 5), 0, &"ally_matrix")
	var state := CombatState.new(grid, [victim, ally])
	...
	assert_true(grid.field_items.has(drop_cell))
	var ejected: MatrixCore = grid.field_items[drop_cell][0]
	...
	assert_eq(ally.held_core, ejected)
