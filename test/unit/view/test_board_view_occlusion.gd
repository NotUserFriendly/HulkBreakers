...
## active unit... fade it." Same geometry the wall-cutout tests already
## proved occludes (unit at (2,6), blocker at (2,3), camera at
## (2,5,-5) looking at the unit) — reusing `WallLegibility.
## occludes_on_screen` unchanged, just against `aim_active_unit`.
func test_update_friendly_fade_ghosts_a_friendly_blocking_the_active_units_aim() -> void:
	var view := BoardView.new()
	add_child_autofree(view)
	...
	add_child_autofree(camera)
	camera.global_position = Vector3(2, 5, -5)
	camera.look_at(active_position, Vector3.UP)

	view.update_friendly_fade(camera)

	assert_eq(
		view._friendly_fade_overlay.get_child_count(),
		1,
		"a friendly standing on the camera's own line of sight to the active unit must be ghosted"
	)


func test_update_friendly_fade_never_ghosts_the_active_unit_itself() -> void:
	var view := BoardView.new()
	add_child_autofree(view)
	view.build(Grid.new(5, 8), DataLibrary.material_table())

	var active := _torso_unit(Vector2i(2, 6), 0)
	view.wall_cutout_units = [active]
	view.aim_active_unit = active
	...
	assert_eq(
		view._friendly_fade_overlay.get_child_count(), 0, "the active unit must never ghost itself"
	)


func test_update_friendly_fade_does_not_ghost_an_enemy_in_the_same_spot() -> void:
	var view := BoardView.new()
	add_child_autofree(view)
	view.build(Grid.new(5, 8), DataLibrary.material_table())

	var active := _torso_unit(Vector2i(2, 6), 0)
	var enemy := _torso_unit(Vector2i(2, 3), 1)
	view.wall_cutout_units = [active, enemy]
	view.aim_active_unit = active
	var active_position: Vector3 = UnitGeometry.bounding_sphere(active).center
	...
	assert_eq(
		view._friendly_fade_overlay.get_child_count(),
		0,
		"an enemy in the blocker's own spot must not be ghosted — friendly-only"
	)


func test_update_friendly_fade_clears_when_aim_active_unit_is_cleared() -> void:
	var view := BoardView.new()
	add_child_autofree(view)
	view.build(Grid.new(5, 8), DataLibrary.material_table())
	...
	view.update_friendly_fade(camera)
	assert_eq(view._friendly_fade_overlay.get_child_count(), 1, "sanity: fade fired while aiming")

	view.aim_active_unit = null
	view.update_friendly_fade(camera)

	assert_eq(
		view._friendly_fade_overlay.get_child_count(), 0, "leaving aim must restore full opacity"
	)


func test_friendly_fade_material_is_gray_and_translucent_not_team_colored() -> void:
	var view := BoardView.new()
	add_child_autofree(view)
	view.build(Grid.new(5, 8), DataLibrary.material_table())
	...
	var instance: MeshInstance3D = view._friendly_fade_overlay.get_child(0)
	var material: StandardMaterial3D = instance.mesh.material
	assert_eq(material.transparency, BaseMaterial3D.TRANSPARENCY_ALPHA)
	assert_almost_eq(material.albedo_color.a, BoardView.FRIENDLY_FADE_ALPHA, 0.001)
	...
	assert_ne(
		...
		"must be gray-tinted, not the team-color ghost"
	)
